...
{
    HeeksObjDlg::SetPicture(theApp.GetDialogBitmapPath(name, _T("pattern")));
}
...
void PatternDlg::RedrawBitmap(wxBitmap &bitmap, long numA, long numB, double xshiftA, double yshiftA, double xshiftB, double yshiftB)
{
	...
	wxImage img(theApp.GetDialogBitmapPath(_T("shape"), _T("pattern")), wxBITMAP_TYPE_PNG);
	wxImage img2(theApp.GetDialogBitmapPath(_T("shape2"), _T("pattern")), wxBITMAP_TYPE_PNG);
