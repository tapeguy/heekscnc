...
void CToolDlg::GetDataRaw(HeeksObj* object)
{
	...
	((CTool*)object)->m_params.m_diameter = m_dblDiameter->GetValueAsDouble();
	((CTool*)object)->m_params.m_tool_length_offset = m_dblToolLengthOffset->GetValueAsDouble();
	((CTool*)object)->m_params.m_flat_radius = m_dblFlatRadius->GetValueAsDouble();
	((CTool*)object)->m_params.m_corner_radius = m_dblCornerRadius->GetValueAsDouble();
	((CTool*)object)->m_params.m_cutting_edge_angle = m_dblCuttingEdgeAngle->GetValueAsDouble();
	((CTool*)object)->m_params.m_cutting_edge_height = m_dblCuttingEdgeHeight->GetValueAsDouble();
	((CTool*)object)->SetTitle ( m_txtTitle->GetValue() );
...
void CToolDlg::SetFromDataRaw(HeeksObj* object)
{
	...
	m_dlbToolNumber->SetValue(wxString::Format(_T("%d"), tool_number));
	m_cmbMaterial->SetSelection(((CTool*)object)->m_params.m_material);
	m_cmbToolType->SetSelection(((CTool*)object)->m_params.m_type);
	m_dblDiameter->SetValueFromDouble(((CTool*)object)->m_params.m_diameter);
	m_dblToolLengthOffset->SetValueFromDouble(((CTool*)object)->m_params.m_tool_length_offset);
	m_dblCuttingEdgeHeight->SetValueFromDouble(((CTool*)object)->m_params.m_cutting_edge_height);
	m_txtTitle->SetValue(((CTool*)object)->GetTitle());
	m_cmbTitleType->SetSelection(((CTool*)object)->m_params.m_automatically_generate_title ? 1:0);

	int type = ((CTool*)object)->m_params.m_type;
	EnableAndSetCornerFlatAndAngle((CToolParams::eToolType)type);
}

void CToolDlg::SetPicture(const wxString& name)
{
	HeeksObjDlg::SetPicture(theApp.GetDialogBitmapPath(name, _T("ctool")));
}
...
void CToolDlg::EnableAndSetCornerFlatAndAngle(CToolParams::eToolType type)
{
	switch(type)
	{
		...
		case CToolParams::eCentreDrill:
			...
			m_dblCuttingEdgeAngle->SetValueFromDouble(((CTool*)m_object)->m_params.m_cutting_edge_angle);
			break;
		case CToolParams::eEndmill:
		case CToolParams::eSlotCutter:
			m_dblCornerRadius->Enable();
			m_dblCornerRadius->SetValueFromDouble(((CTool*)m_object)->m_params.m_corner_radius);
			m_dblFlatRadius->Enable(false);
			m_dblFlatRadius->SetLabel(_T(""));
		...
		case CToolParams::eEngravingTool:
			...
			m_dblFlatRadius->SetValueFromDouble(((CTool*)m_object)->m_params.m_flat_radius);
			m_dblCuttingEdgeAngle->Enable();
			m_dblCuttingEdgeAngle->SetValueFromDouble(((CTool*)m_object)->m_params.m_cutting_edge_angle);
			break;
		default:
